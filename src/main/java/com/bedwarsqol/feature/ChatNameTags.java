 ...
 * Forge and Weave alike: Weave's hook renders from the original component reference (not the event's
 * final message field), so an in-place mutation is seen while a reassignment would be ignored. Inserting
 * the FKDR at sibling index 0 lands it at the very front because Hypixel's formatted chat lines carry an
 * empty-text root with all content in siblings. Prepending ahead of the name also keeps us clear of
 * Hypixel's rank-card hover, which hangs off the name component. (Lunar can still detach the <i>stored</i>
 * line from this reference by storing a converted copy at add time — {@link #ensureVisible} detects and
 * repairs that after each tag write.)
 *
...
public final class ChatNameTags {
    ...
    public void onChat(ClientChatReceivedEvent event) {
        ...
        // as lone usernames and would get a bogus FKDR bracket. See ModChat.
        ...
        String sender = ChatSender.extractName(event.message);
        if (sender == null) return;
        if (isSelf(sender)) return;
        boolean typedShape = ChatSender.typedChatName(event.message) != null;
