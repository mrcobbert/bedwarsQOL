...
public class NametagStats {
    ...
    @SubscribeEvent
    public void onRenderNameTag(RenderLivingEvent.Specials.Post<EntityPlayer> event) {
    ...
    private void drawStat(AbstractClientPlayer player, RenderLivingEvent.Specials.Post<EntityPlayer> event, String text) {
        float above = ABOVE_NAMETAG;
        if (hasBelowNameLine(player)) above += BELOW_NAME_SHIFT;
        drawLineAbove(player, event.x, event.y, event.z, text, above);
    }
