...
public class SettingsGui extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        // Re-establish the 2D GUI orthographic projection ourselves, every frame, before anything draws.
        // Why this is required: while the settings GUI is open, BedwarsHudRenderer cancels the in-game
        // overlay (to hide the HUD behind the panel). In 1.8.9, when a world is loaded, that overlay
        // (GuiIngameForge.renderGameOverlay) is the ONLY place vanilla calls EntityRenderer
        // .setupOverlayRendering() before currentScreen.drawScreen — and it does so AFTER the cancellable
        // Pre(ALL) event. Cancelling that event makes renderGameOverlay return early, so the projection is
        // never set and drawScreen would otherwise inherit the 3D world camera matrices, projecting every
        // GuiRender quad off-screen (the whole panel goes invisible). Setting it up here makes the panel
        // render correctly regardless of whether the overlay ran.
        net.minecraft.client.gui.ScaledResolution sr = new net.minecraft.client.gui.ScaledResolution(mc);
        mc.entityRenderer.setupOverlayRendering();
        // Blur the world ONLY behind the panel (frosted-glass), leaving the surrounding game view sharp.
        // The clip rect is the panel in real window pixels (glScissor form: origin bottom-left).
        float eff = sr.getScaleFactor() * uiScale;
        int sx = Math.round(panelX * eff);
        int sy = Math.round(mc.displayHeight - (panelY + panelH) * eff);
        int sw = Math.round(panelW * eff);
        int sh = Math.round(panelH * eff);
        GuiBlur.update(sx, sy, sw, sh); // frosted backdrop confined to the panel
        advanceScroll();
    ...
    private static void toggle(ClientSettings cfg, int kind) {
        switch (kind) {
            ...
            case K_CHATHEADS:
                ...
                // Rebuild chat so the reserved head slots appear/disappear at once (see ChatPlayerHeads).
